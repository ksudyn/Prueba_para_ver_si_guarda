/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_parse_tokens.c                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ksudyn <ksudyn@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/19 20:11:16 by ksudyn            #+#    #+#             */
/*   Updated: 2025/12/26 20:09:25 by ksudyn           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/miniRT.h"

// Cuenta cuántas palabras hay en una línea separadas por espacios
// static int	count_words(char *line)
// {
// 	int		count;
// 	bool	inside_word;

// 	count = 0;
// 	while (line && *line)
// 	{
// 		inside_word = false;
// 		while (ft_isspace(*line) && *line)
// 			line++;
// 		while (!ft_isspace(*line) && *line)
// 		{
// 			if (!inside_word)
// 			{
// 				count++;
// 				inside_word = true;
// 			}
// 			line++;
// 		}
// 	}
// 	return (count);
// }

// // Obtiene la siguiente palabra de una línea
// static char	*get_next_word(char *line, int *cursor)
// {
// 	char	*word;
// 	int		length;
// 	int		i;

// 	length = 0;
// 	i = 0;
// 	while (ft_isspace(line[*cursor]))
// 		(*cursor)++;
// 	while (line[*cursor + length] && !ft_isspace(line[*cursor + length]))
// 		length++;
// 	word = malloc(sizeof(char) * (length + 1));
// 	if (!word)
// 		return (NULL);
// 	while (line[*cursor] && !ft_isspace(line[*cursor]))
// 		word[i++] = line[(*cursor)++];
// 	word[i] = '\0';
// 	return (word);
// }

// // Divide la línea en tokens separados por espacios
// char	**split_line_by_space(char *line)
// {
// 	int		words_count;
// 	char	**tokens;
// 	int		i;
// 	int		cursor;

// 	i = 0;
// 	cursor = 0;
// 	if (!line)
// 		return (free(line), NULL);
// 	words_count = count_words(line);
// 	tokens = malloc(sizeof(char *) * (words_count + 1));
// 	if (!tokens)
// 		return (free(line), NULL);
// 	while (words_count-- > 0)
// 		tokens[i++] = get_next_word(line, &cursor);
// 	tokens[i] = NULL;
// 	free(line);
// 	return (tokens);
// }

// char	**read_next_line_tokens(int fd)
// {
// 	char	*line;

// 	line = get_next_line(fd);
// 	return (split_line_by_space(line));
// }


// Cuenta las palabras separadas por espacios
static int count_words(const char *line)
{
    int count = 0;
    bool inside_word = false;

    if (!line)
        return 0;

    while (*line)
    {
        if (ft_isspace(*line))
            inside_word = false;
        else if (!inside_word)
        {
            count++;
            inside_word = true;
        }
        line++;
    }
    return count;
}

// Obtiene la siguiente palabra de una línea
static char *get_next_word(const char *line, int *cursor)
{
    int start, end;
    char *word;

    if (!line || !cursor)
        return NULL;

    // Saltar espacios iniciales
    while (line[*cursor] && ft_isspace(line[*cursor]))
        (*cursor)++;

    start = *cursor;

    while (line[*cursor] && !ft_isspace(line[*cursor]))
        (*cursor)++;

    end = *cursor;

    if (end <= start)
        return NULL;

    word = malloc(end - start + 1);
    if (!word)
        return NULL;

    for (int i = 0; i < end - start; i++)
        word[i] = line[start + i];
    word[end - start] = '\0';

    return word;
}

// Divide la línea en tokens separados por espacios
char **split_line_by_space(char *line)
{
    int words_count;
    char **tokens;
    int cursor = 0;

    if (!line)
        return NULL;

    words_count = count_words(line);
    tokens = malloc(sizeof(char *) * (words_count + 1));
    if (!tokens)
        return NULL;

    for (int i = 0; i < words_count; i++)
    {
        tokens[i] = get_next_word(line, &cursor);
        if (!tokens[i])
        {
            // Liberar memoria en caso de error
            for (int j = 0; j < i; j++)
                free(tokens[j]);
            free(tokens);
            return NULL;
        }
    }
    tokens[words_count] = NULL;
    free(line);
    return tokens;
}


// Lee la siguiente línea del archivo y devuelve tokens
char **read_next_line_tokens(int fd)
{
    char *line = get_next_line(fd);
    printf("DEBUG: read_next_line_tokens: got line ptr=%p\n", (void*)line);
    if (!line)
        return NULL;
    char **tokens = split_line_by_space(line);
    printf("DEBUG: read_next_line_tokens: tokens=%p\n", (void*)tokens);
    return tokens;
}

